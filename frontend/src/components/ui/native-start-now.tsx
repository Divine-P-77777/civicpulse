...
export function NativeStartNow({
  ...
  const getButtonStyles = () => {
    const baseStyles =
      "relative overflow-hidden font-semibold transition-all duration-300 flex items-center justify-center";
    ...
    switch (variant) {
      case "gradient":
        return cn(
          ...
          "hover:shadow-[0_4px_14px_rgba(79,70,229,0.3)]",
          "border-0"
        );
      case "solid":
        return cn(
        ...
        {showRocket && isHovered && status === "idle" && (
          <>
            {[...Array(6)].map((_, i) => (
              ...
              >
                <div className="h-3 w-3 text-indigo-600 fill-indigo-600">
                  {icon || (
                    <Rocket className="h-3 w-3 text-indigo-400 fill-indigo-400" />
                  )}
                </div>
              </motion.div>
            ))}
