...
const ChatMessage = memo(function ChatMessage({ role, content, timestamp, isStreaming }: ChatMessageProps) {
    ...
    return (
        <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} message-slide-in`}>
            <div className={`flex items-start gap-3 max-w-[85%] md:max-w-2xl ${isUser ? 'flex-row-reverse' : ''}`}>
                {/* Avatar */}
                ...
                <div className={`rounded-2xl px-4 py-3 ${isUser
                    ...
                    {isUser ? (
                        ...
                        <div className="text-sm leading-relaxed">
                            <p className="whitespace-pre-wrap">{content}<span className="text-[#2A6CF0] animate-pulse font-bold">▌</span></p>
                        </div>
                    ) : (
